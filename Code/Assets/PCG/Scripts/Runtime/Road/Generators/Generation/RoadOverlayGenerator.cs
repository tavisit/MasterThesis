...
public sealed class RoadOverlayGenerator : MonoBehaviour
{
    ...
    public void ProcessSpline(SplineContainer container, RoadType roadType)
    {
        ...
        if (_generateTunnels)
        {
            PlaceTunnelArches(
                ...
                spline,
                length,
                hw * Mathf.Max(1f, _archWidthMultiplier),
                forceBorehole: roadType == RoadType.Metro);
        }
    ...
    private void PlaceTunnelArches(
        ...
        bool forceBorehole)
    {
        ...
        var localPositions = new Vector3[samples];
        var worldPositions = new Vector3[samples];
        ...
        for (int i = 0; i < samples; i++)
        {
            ...
            localPositions[i] = pos3;
            worldPositions[i] = container.transform.TransformPoint(localPositions[i]);
            tangents[i] = ((Vector3)tan3).normalized;

            float terrainH = _terrain != null
                ? _terrain.SampleHeight(worldPositions[i].x, worldPositions[i].z)
                : worldPositions[i].y;

            inTunnel[i] = forceBorehole || terrainH - worldPositions[i].y > _tunnelHeightThreshold;
        }
        ...
        foreach (int entry in entryPoints)
        {
            ...
            {
                inTunnel[k] = true;
                rem -= k > 0 ? Vector3.Distance(worldPositions[k - 1], worldPositions[k]) : 0f;
            }
        }
        ...
        foreach (int exit in exitPoints)
        {
            ...
            {
                inTunnel[k] = true;
                rem -= k < samples - 1 ? Vector3.Distance(worldPositions[k], worldPositions[k + 1]) : 0f;
            }
        }
        ...
        for (int i = 0; i < samples; i++)
        {
            ...
            float prevDist = i > 0
                ...
                : Vector3.Distance(worldPositions[i], worldPositions[Mathf.Min(i + 1, samples - 1)]);
            float nextDist = i < samples - 1
                ? Vector3.Distance(worldPositions[i], worldPositions[i + 1])
                : prevDist;
            ...
            Vector3 safeForward = ResolveSafeForward(i, tangents, worldPositions);
            go.transform.rotation = Quaternion.LookRotation(safeForward, Vector3.up);
    ...
    private static Vector3 ResolveSafeForward(int index, Vector3[] tangents, Vector3[] worldPositions)
    {
        ...
        {
            if (index > 0 && index < worldPositions.Length)
            {
                Vector3 back = worldPositions[index] - worldPositions[index - 1];
            ...
            if (index >= 0 && index < worldPositions.Length - 1)
            {
                Vector3 fwd = worldPositions[index + 1] - worldPositions[index];
