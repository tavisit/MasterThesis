...
namespace Assets.Scripts.Runtime.Road.Generators
{
    ...
    {
        private const float MetroDepthBelowTerrain = 8f;

        public static List<SplineContainer> Generate(
            RoadGraph streetGraph,
            CityNucleus[] nuclei,
            Transform parent,
            Assets.Scripts.Runtime.Adapters.TerrainAdapter terrainAdapter,
            ...
            int maxLines = 1)
        {
            ...
            var dsuParent = new int[nuclei.Length];
            for (int i = 0; i < nuclei.Length; i++)
            {
                dsuParent[i] = i;
            }
            ...
            int Find(int x)
            {
                while (dsuParent[x] != x)
                {
                    x = dsuParent[x] = dsuParent[dsuParent[x]];
                }
            ...
            foreach (var (_, i, j) in allPairs)
            {
                ...
                SplineContainer container = path != null && path.Count > 0
                    ? BuildSplineFromPath(path, parent, terrainAdapter, morphology)
                    : BuildDirectSpline(nucleusNodes[i].Position, nucleusNodes[j].Position,
                                        parent, terrainAdapter, morphology);
            ...
            UrbanMorphology morphology)
        {
            ...
            {
                float y = terrainAdapter != null
                    ? terrainAdapter.SampleHeight(p.x, p.z) - MetroDepthBelowTerrain
                    : p.y - MetroDepthBelowTerrain;
